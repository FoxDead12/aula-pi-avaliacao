...
export class DocenteService implements IDocenteService {
  constructor(
    ...
    @InjectRepository(DtoDocente)
    private docenteRepository: Repository<DtoDocente>,
    @InjectRepository(DtoHorarioDocente)
    private horarioDocenteRepository: Repository<DtoHorarioDocente>,
  ) {}
  
  async InsertAllHorarios(data: IDocente[]): Promise<void> {
    const dtos: DtoHorarioDocente[] = [];
    let id = 0;
    data.forEach(docente => {
      const currentDocente: DtoDocente = {
        id: docente.id,
        nome: docente.nome,
        horarioDocente: null
      ...
      docente.horario.segunda.forEach(aula => {

        const dto: DtoHorarioDocente = {
          diaSemana: 1,
      ...
      docente.horario.terca.forEach(aula => {
      ...
      docente.horario.quarta.forEach(aula => {
      ...
      docente.horario.quinta.forEach(aula => {
      ...
      docente.horario.sabado.forEach(aula => {
    ...
    await this.horarioDocenteRepository.save(dtos);
  }
  async InsertAllDocentes(data: IDocente[]): Promise<void> {
    const dtos: DtoDocente[] = [];
    data.forEach(turma => {
      const dto: DtoDocente = {
        horarioDocente: null,
        id: turma.id,
        nome: turma.nome
      }
    ...
    await this.docenteRepository.save(dtos);
  }
